...
class VillagesLayer {
    ...
    description(feature) {
        var obj = el('.villages-object',
            el('h3', 
                el('a', feature.properties.name,
                    {
                        target: '_blank',
                        href: this._wiki_url + encodeURIComponent(feature.properties.wiki_page)
                }),
            )
        )
    ...
    createForm(data={}, edit=false) {
        ...
        editor.onSubmit = (e) => this.post_data(endpoint, editor.getData(), method)
            .then(resp => {
                if (resp.status == 200) {
                    closeDialog()
                    this.refreshLayer()
        ...
        editor.onDelete = (e) => this.post_data(data.id, "", "DELETE")
            .then(resp => {
                if (resp.status == 200) {
                     closeDialog()
                     this.refreshLayer()
